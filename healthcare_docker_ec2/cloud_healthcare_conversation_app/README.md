...
If Docker previously failed while building `openai-whisper`, rebuild after the updated Dockerfile is saved. The Dockerfile now installs `setuptools` and uses `--no-build-isolation`, which avoids the `pkg_resources` build error.
